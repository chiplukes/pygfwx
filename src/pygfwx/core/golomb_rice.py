...
def unsigned_decode(
    pot: int, stream: BitReader
) -> int:  # cm:f8a9b0 — unsigned_decode(): Golomb-Rice decode (run + remainder)
    """
...
def interleaved_decode(
    pot: int, stream: BitReader
) -> int:  # cm:c1d2e3 — interleaved_decode(): signed via zig-zag mapping
    """
...
def unsigned_encode(
    pot: int, value: int, stream: BitWriter
) -> None:  # cm:c7d8e9 — unsigned_encode(): Golomb-Rice encode with escape
    """
...
def interleaved_encode(
    pot: int, value: int, stream: BitWriter
) -> None:  # cm:f0a1b2 — interleaved_encode(): signed via zig-zag mapping
    """
...
def signed_encode(
    pot: int, value: int, stream: BitWriter
) -> None:  # cm:c3d4e5b — signed_encode(): magnitude + sign-bit coding
    """
