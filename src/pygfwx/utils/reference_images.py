...
def create_reference_image(
    size: int = 64, channels: int = 1, bit_depth: int = 8
) -> (
    np.ndarray
):  # cm:a7b8c9d — create_reference_image(): canonical test image (4-quadrant: smooth/stripes/checker/mixed)
...
def create_uniform_image(
    size: int = 64, value: int = 128, channels: int = 1, bit_depth: int = 8
) -> np.ndarray:  # cm:e0f1a2b — create_uniform_image(): constant-value image (DC and quantization edge cases)
...
def create_impulse_image(
    size: int = 64, channels: int = 1, bit_depth: int = 8
) -> np.ndarray:  # cm:c3d4e5c — create_impulse_image(): single bright pixel (impulse response / boundary tests)
